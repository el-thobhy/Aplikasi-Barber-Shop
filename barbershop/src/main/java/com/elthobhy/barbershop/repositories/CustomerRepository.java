...
public interface CustomerRepository extends JpaRepository<MasterCustomer, Integer> {
    @Modifying
    @Query(value = "UPDATE master_customer SET deleted=true, update_by=:updateBy, update_date=now() WHERE id=:id", nativeQuery = true)
    public void softDelete(@Param("id") Integer id, @Param("updateBy") Integer updateBy);

    Optional<List<MasterCustomer>> findByDeleted(boolean deleted);
}
